...
export class TranslationService {
  ...
  batch(texts) {
    ...
    for( var i=0; i <texts.length; i++) {
      results.push(this.free(texts[i]))
    }
  ...
  request(text) {
    ...
    const attempts = () => new Promise((resolve, reject) => {
      // If the api call fails we reject it otherwise we resolve. 
      this.api.request(text, (res) => {
        res ? reject(res) : resolve(res)
      })
    })

    return attempts().catch(attempts).catch(attempts)
  }
